...
const NewSimpleGridUI: React.FC = () => {
  ...
  const [selectedFunction, setSelectedFunction] = useState<ethers.utils.FunctionFragment | null>(null);
  ...
  const [contractConnection, setContractConnection] = useState<ContractConnectorResult | null>(null);
  ...
  const handleContractConnected = useCallback((result: ContractConnectorResult) => {
    ...
    if (result.tokenInfo?.type === 'unknown' && result.abi) {
      ...
      const diamondFunctions = ['facets', 'facetAddresses', 'facetAddress', 'facetFunctionSelectors'];
      const hasDiamondFunctions = diamondFunctions.some(funcName =>
        result.abi.some((item: any) => item.type === 'function' && item.name === funcName)
      );
  ...
  const loadDiamondFacets = useCallback(async (address: string, chain: Chain) => {
    setFacetLoading(true);
    setFacetProgress({ current: 0, total: 0, currentFacet: '', status: 'fetching' });
    ...
    try {
      ...
      const facets = await fetchDiamondFacets(chain, facetAddresses, progressCallback);
  ...
  // Function selection
  const handleFunctionSelected = useCallback((func: ethers.utils.FunctionFragment, type: 'read' | 'write') => {
  ...
  // Function execution
  const handleFunctionExecuted = useCallback((result: any) => {
    setExecutionResult(result);
    setIsExecuting(false);
    
    // If this was a write function and we got a transaction hash, extract calldata for decoder
    if (result.success && selectedFunction && selectedFunctionType === 'write') {
      try {
        if (contractConnection?.interface && selectedFunction) {
          const calldata = contractConnection.interface.encodeFunctionData(selectedFunction);
  ...
  const getFilteredFunctions = useCallback((type: 'read' | 'write') => {
    if (!contractConnection) return [];
    
    const baseFunctions = type === 'read' ? contractConnection.readFunctions : contractConnection.writeFunctions;
  ...
  const getSearchFilteredFunctions = useCallback((type: 'read' | 'write') => {
    ...
    <div style={{
      ...
      <div style={gridStyle}>
              ...
              <Card title="💎 Diamond Facets" variant="accent" padding="sm">
                ...
                {facetLoading && contractConnection && (
                  <div style={{ padding: '8px', fontSize: '12px', color: '#888' }}>
                    Loading facets... {facetProgress.current}/{facetProgress.total}
                    {facetProgress.currentFacet && ` (${facetProgress.currentFacet})`}
                  </div>
                )}
                    ...
                    {diamondFacets.map((facet, index) => (
                      <div
                        ...
                        style={{
                          padding: '8px',
                          margin: '4px 0',
                          background: selectedFacet === facet.address ? 'rgba(0, 123, 255, 0.2)' : 'rgba(255, 255, 255, 0.05)',
                          borderRadius: '6px',
                          cursor: 'pointer',
                          fontSize: '12px',
                        }}
                        onClick={() => setSelectedFacet(selectedFacet === facet.address ? null : facet.address)}
                      >
                        <div style={{ fontWeight: '500' }}>{facet.name || `Facet ${index + 1}`}</div>
        ...
        <div style={cardStyle}>
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <div style={{
                  ...
                  borderRadius: '50%',
                  border: '2px solid #007bff',
                  background: functionMode === 'function' ? '#007bff' : 'transparent',
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
                }}>
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <div style={{
                  ...
                  borderRadius: '50%',
                  border: '2px solid #007bff',
                  background: functionMode === 'raw' ? '#007bff' : 'transparent',
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
                }}>
          ...
          {functionMode === 'function' && isConnected && contractConnection && (
            <div style={{ marginBottom: '24px' }}>
              ...
              <div style={{ marginBottom: '16px', display: 'flex', gap: '8px' }}>
                ...
                {(getFilteredFunctions('read').length + getFilteredFunctions('write').length) > 0 && (
                  <Badge variant="info" size="sm">
                    {getFilteredFunctions('read').length} read, {getFilteredFunctions('write').length} write
                  </Badge>
                )}
              </div>
              ...
              {getSearchFilteredFunctions('read').length > 0 && (
                <div style={{ marginBottom: '16px' }}>
                  <div style={{ marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <h4 style={{ margin: 0, fontSize: '14px', fontWeight: '500' }}>Read Functions</h4>
                    <Badge variant="info" size="sm">{getSearchFilteredFunctions('read').length}</Badge>
                  </div>
                  <div style={{ display: 'grid', gap: '4px' }}>
                    {getSearchFilteredFunctions('read').map((func, index) => (
                      <button
                        ...
                        style={{
                          padding: '8px 12px',
                          background: selectedFunction?.name === func.name && selectedFunctionType === 'read' 
                            ? 'rgba(0, 123, 255, 0.2)' 
                            : 'rgba(255, 255, 255, 0.05)',
                          border: '1px solid #333',
                          borderRadius: '6px',
              ...
              {getSearchFilteredFunctions('write').length > 0 && (
                <div>
                  <div style={{ marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <h4 style={{ margin: 0, fontSize: '14px', fontWeight: '500' }}>Write Functions</h4>
                    <Badge variant="warning" size="sm">{getSearchFilteredFunctions('write').length}</Badge>
                  </div>
                  <div style={{ display: 'grid', gap: '4px' }}>
                    {getSearchFilteredFunctions('write').map((func, index) => (
                      <button
                        ...
                        style={{
                          padding: '8px 12px',
                          background: selectedFunction?.name === func.name && selectedFunctionType === 'write' 
                            ? 'rgba(255, 193, 7, 0.2)' 
                            : 'rgba(255, 255, 255, 0.05)',
                          border: '1px solid #333',
                          borderRadius: '6px',
          ...
          {/* Function Execution */}
          {functionMode === 'function' && isConnected && selectedFunction && contractConnection && (
            <div style={{ marginBottom: '24px' }}>
              <FunctionCaller
