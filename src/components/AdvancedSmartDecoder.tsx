...
import { FileText, Target, BarChart3, RotateCw, Building2 } from "lucide-react";
...
const AdvancedSmartDecoder: React.FC = () => {
  ...
  const [heuristicResult, setHeuristicResult] =
    useState<HeuristicDecodingResult | null>(null);
  ...
  const [enableStructVisualization, setEnableStructVisualization] =
    useState(true);
  ...
  const [viewMode, setViewMode] = useState<"advanced" | "legacy" | "simple">(
    "advanced"
  );
  ...
  const parseSignatureParameters = (
    signature: string
  ...
  const handleAdvancedDecode = async () => {
    ...
    try {
      ...
      if (customSignature) {
        ...
        const enhancedParams = enhanceParameterVisualization(
          decoded,
          customSignature
        );
      ...
      try {
        const openChainResult: SignatureResponse =
          await lookupFunctionSignatures([selector]);
      ...
      if (enableHeuristics) {
        ...
        try {
          ...
          if (heuristicResults && heuristicResults.bestGuess) {
            addDecodingStep(
              `Best guess: ${heuristicResults.bestGuess.description} (confidence: ${(heuristicResults.bestGuess.confidence * 100).toFixed(1)}%)`
            );
          }

          addDecodingStep(
            `Generated ${heuristicResults?.decodedAttempts?.length || 0} possible decodings`
          );
        } catch (heuristicError) {
          console.error("Heuristic decoding error:", heuristicError);
          addDecodingStep(
            ` Heuristic decoding failed: ${String(heuristicError)}`
          );
          setHeuristicResult(null);
        }
      }
      ...
      if (!heuristicResult?.bestGuess) {
        addDecodingStep(
          " Function signature not found in databases - try heuristic results above"
        );
      }
    } catch (err: any) {
  ...
  const enhanceParameterVisualization = (
    decoded: any,
    signature: string
  ): DecodedParameter[] => {
    ...
    return decoded.args.map((arg: any, index: number) => {
      const paramInfo = parameterInfo[index] || {
        name: `arg${index}`,
        type: "unknown",
      };
      ...
      if (
        enableStructVisualization &&
        detectStructPattern([arg], [paramInfo.type])
      ) {
    ...
    return (
      <div className="heuristic-results">
        ...
        {heuristicResult.bestGuess && (
          <div className="best-guess">
            <h5>
              ...
              Best Guess (
              {(heuristicResult.bestGuess.confidence * 100).toFixed(1)}%
              confidence)
            </h5>
            <div className="guess-details">
              ...
              <div className="decoded-values">
                ...
                {shouldUseComplexViewer(
                  heuristicResult.bestGuess.values || []
                ) ? (
                  <DecodedDataViewer
                  ...
                  (heuristicResult.bestGuess.values || []).map(
                    (value, index) => (
                      <div key={index} className="decoded-value">
                        ...
                        {Array.isArray(value) && value.length > 0 ? (
                          renderExpandableArray(
                            value,
                            `heuristic-best-${index}`,
                            heuristicResult.bestGuess!.types?.[index] ||
                              "unknown"
                          )
                        ) : (
                          <code>
        ...
        <div className="all-attempts">
          ...
          {(heuristicResult.decodedAttempts || [])
            .slice(1)
            .map((attempt, index) => (
              <details key={index} className="attempt-details">
                <summary>
                  {attempt.description} ({(attempt.confidence * 100).toFixed(1)}
                  % confidence)
                </summary>
                      ...
                      (attempt.values || []).map((value, idx) => (
                        <div key={idx} className="decoded-value">
                          ...
                          {Array.isArray(value) && value.length > 0 ? (
                            renderExpandableArray(
                              value,
                              `heuristic-attempt-${index}-${idx}`,
                              attempt.types?.[idx] || "unknown"
                            )
                          ) : (
                            <code>
                              {formatParameterValue(
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          <span>
            ...
            {value
              .map((v) =>
                Array.isArray(v)
                  ? `[Array of ${v.length} items]`
                  : formatSingleValue(v, type?.replace("[]", "") || "")
              )
              .join(", ")}
            ]
          </span>
          <button
            onClick={() =>
              copyToClipboard(JSON.stringify(value, null, 2), "array")
            }
            style={{
              background: "none",
        ...
        {isExpanded ? (
          ...
          >
            ...
            {value.map((item, index) => (
              ...
              >
                <span style={{ color: "#6b7280", minWidth: "30px" }}>
                  [{index}]:
                </span>
                <span style={{ flex: 1, wordBreak: "break-all" }}>
                  {Array.isArray(item)
                    ? `[Array of ${item.length} items]`
          ...
          <span
            style={{ fontSize: "12px", fontFamily: "Monaco, Menlo, monospace" }}
          >
            [
            {value
              .slice(0, 2)
  ...
  const formatParameterValue = (value: any, type: string): string => {
    if (enableArrayHelpers && Array.isArray(value)) {
      if (value.length <= 3) {
        ...
        return `[${value
          .slice(0, 2)
          .map((v) => formatSingleValue(v, type.replace("[]", "")))
          .join(", ")}, ... +${value.length - 2} more]`;
      }
    }
  ...
  return (
    <div className="advanced-smart-decoder">
      <h2> Advanced Smart Decoder</h2>
      <p>
        Enhanced decoder with heuristic analysis, struct visualization, and
        event log filtering.
      </p>
              ...
              <div className="grid-container grid-3-col">
                ...
                <label>
                  <input
                    ...
                    onChange={(e) =>
                      setEnableStructVisualization(e.target.checked)
                    }
                  />
          ...
          {decodedResult && (
            <div className="decode-result">
              ...
              <div className="result-section">
                ...
                {decodedResult.enhancedParams &&
                decodedResult.enhancedParams.length > 0 ? (
                  <div className="result-field">
                    <div className="action-bar">
                    ...
                    {viewMode === "advanced" && (
                      <AdvancedJsonEditor
                        data={
                          decodedResult.enhancedParams?.map(
                            (param: DecodedParameter) => param.value
                          ) || []
                        }
                        onChange={() => {}} // Read-only for decoder results
                        functionInputs={decodedResult.enhancedParams?.map(
                    ...
                    {viewMode === "legacy" && (
                      <div className="enhanced-params">
                        <DecodedDataViewer
                          data={decodedResult.enhancedParams.map(
                            (param: DecodedParameter) => param.value
                          )}
                          types={decodedResult.enhancedParams.map(
                            (param: DecodedParameter) => param.type
                          )}
                          paramNames={decodedResult.enhancedParams.map(
                    ...
                    {viewMode === "simple" && (
                      <div className="etherscan-args-display">
                        ...
                        {decodedResult.enhancedParams.map(
                          (param: DecodedParameter, index: number) => (
                            <div
                              key={index}
                              className="arg-row"
                              style={{
          ...
          {showTransferOptions && (
            <div className="form-section">
              ...
              <div className="horizontal-pane center">
                ...
                <small>
                  Create a similar transaction with modified parameters
                </small>
              </div>
            </div>
          )}
          ...
          <div className="form-section">
            ...
            <div className="input-group horizontal">
              ...
              >
                {isLoadingEvents ? (
                  <>
                    <RotateCw size={16} className="inline mr-2 animate-spin" />
                    Searching...
                  </>
                ) : (
                  <>
                    ...
                    {log.args.map((arg, argIndex) => (
                      <div key={argIndex} className="event-arg">
                        <span className="arg-name">
                          {arg.name} ({arg.indexed ? "indexed" : "data"}):
                        </span>
                        <code className="arg-value">
                          {formatEventLogValue(arg.value, arg.type)}
                        </code>
                      </div>
                    ))}
                  </div>
