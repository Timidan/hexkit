...
import { FileMagnifyingGlass, Terminal } from "@phosphor-icons/react";
...
import { setupSolidityMonaco } from "@/lib/monaco";
import { SourceViewPanelShell } from "./shells/SourceViewPanelShell";

interface SourceViewPanelProps {
  className?: string;
}

export const SourceViewPanel: React.FC<SourceViewPanelProps> = React.memo(
  ({ className }) => {
    const {
      sourceFiles,
      currentFile,
    ...
    const monacoBootstrappedRef = useRef(false);
    useEffect(() => {
      if (monacoBootstrappedRef.current) return;
      monacoBootstrappedRef.current = true;
      import("monaco-editor")
        .then((monaco) => setupSolidityMonaco(monaco))
        .catch(() => {});
    }, []);
    ...
    const currentSource = useMemo(() => {
      ...
      if (sourceTexts) {
        if (sourceTexts[currentFile]) {
          return {
            ...
            contractName:
              currentFile.split("/").pop()?.replace(".sol", "") || currentFile,
          };
        }
        ...
        if (filename) {
          for (const [path, content] of Object.entries(sourceTexts)) {
            if (
              ...
              return { path, content, contractName: filename.replace(".sol", "") };
            }
          }
          const contractName = filename.replace(".sol", "");
          for (const [path, content] of Object.entries(sourceTexts)) {
            if (path.includes(contractName + ".sol")) {
              return { path, content, contractName };
            }
          }
        }
      }
      ...
      if (traceContractSource) return traceContractSource;
      return null;
    ...
    const fileBreakpoints = useMemo(() => {
      ...
      return new Set(
        breakpoints
          .filter(
            (bp) =>
              bp.location.type === "source" && bp.location.filePath === currentFile,
          )
    ...
    useEffect(() => {
      const isValidFile =
        currentFile &&
        (sourceTexts?.[currentFile] ||
          sourceFiles.has(currentFile) ||
          currentFile.startsWith("trace://"));
    ...
    if (!currentSource) {
      ...
      const functionName =
        currentFrame?.functionName || currentSimulation?.functionName || "Unknown";
      const currentTraceContract = currentExecutingAddress
        ? traceContracts?.get(currentExecutingAddress)
        : null;
      const contractName =
        currentTraceContract?.name ||
        contractContext?.name ||
        currentFrame?.contractName ||
        "Contract";
      const contractAddress =
        currentExecutingAddress || contractContext?.address || currentFrame?.address;
          ...
          <CardContent className="flex-1 flex flex-col p-4">
            <div className="flex-1 flex flex-col items-center justify-center text-center">
              ...
              {currentExecutingAddress && (
                <p className="text-xs text-muted-foreground/50 mt-2 font-mono">
                  Executing: {currentExecutingAddress.slice(0, 10)}...
                  {currentExecutingAddress.slice(-6)}
                </p>
              )}
            </div>
            ...
            <div className="mt-4 pt-4 border-t space-y-3">
              ...
              <div className="space-y-2 text-xs">
                ...
                {currentSnapshot?.type === "opcode" &&
                  "opcodeName" in currentSnapshot.detail && (
                    <div className="flex justify-between">
    ...
    return (
      <SourceViewPanelShell
        ...
        source={{ content: currentSource.content, language: "solidity" }}
