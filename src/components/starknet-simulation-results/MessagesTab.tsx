// Cross-layer messages tab. Walks the call tree once via collectL2ToL1Messages
    ...
    return (
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card className="p-6 text-sm text-muted-foreground">
          ...
          No messages emitted to L1. Most INVOKE-only transactions don't bridge
          state to L1; this section is populated by contracts calling{" "}
        ...
        <Card className="p-6 text-sm text-muted-foreground">
          ...
          No consumed L1 handler message is present in this simulation
          response. L1 handler transactions will populate this section when
          the bridge exposes consumed-message metadata.
        </Card>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <Card key={i} className="p-3 gap-2 bg-background">
                ...
                <div className="rounded bg-card border border-border p-2 font-mono text-[11px] space-y-0.5">
                  <div className="flex items-center justify-between text-muted-foreground text-[10px] uppercase">
                    <span>
                      payload ({payload.length} felt{payload.length === 1 ? "" : "s"})
                    </span>
                    {payload.length > 0 && (
                      <CopyButton
      ...
      <Card className="p-6 text-sm text-muted-foreground">
        ...
        No consumed L1 handler message is present in this simulation response.
      </Card>
    </div>
  );
}
