...
// Types to avoid any during ABI processing
type StateMutability = "view" | "pure" | "nonpayable" | "payable";
interface AbiParam {
  name: string;
  type: string;
  internalType?: string;
...
interface ExtendedAbiFunction extends AbiFunction {
  facetAddress?: string;
  facetName?: string;
}
...
function isAbiFunction(item: unknown): item is AbiFunction {
  ...
  return (
    ...
    typeof rec["stateMutability"] === "string"
  );
}
...
const UniversalContractInterface: React.FC = () => {
  ...
  const searchContract = async () => {
    ...
    try {
      ...
      if (facetAddresses) {
        ...
        // Analyze facets
        const facetDetails = await analyzeFacets(
          address,
          facetAddresses,
          selectedChain
        );
  ...
  const getFunctions = (): ExtendedAbiFunction[] => {
    ...
    try {
      ...
      const onlyFunctions = parsed.filter(isAbiFunction) as AbiFunction[];
      return onlyFunctions.map((func) => ({
        ...func,
        facetAddress: contractInfo.isDiamond ? selectedFacet : undefined,
        facetName: contractInfo.isDiamond
          ? facets.find((f) => f.address === selectedFacet)?.name
          : undefined,
      }));
    } catch {
      return [];
      ...
      {contractInfo && (
        <div className="max-w-6xl mx-auto px-6 py-6">
          ...
          <div className="bg-gray-800 rounded-lg p-6 mb-6">
            ...
            {/* Facets summary list at bottom of card */}
              ...
              <div className="mt-4 pt-4 border-t border-gray-700">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {facets.map((facet) => (
                    <div
                      key={facet.address}
                      className="flex items-center justify-between bg-gray-700/40 border border-gray-700 rounded-lg px-3 py-2"
                    >
                      <div className="min-w-0">
                        <div className="text-sm text-gray-200 truncate">
                          {facet.name || `Facet ${facet.address.slice(0, 8)}`}
                          {!facet.verified && (
                      ...
                      <div className="text-xs text-gray-300 whitespace-nowrap ml-3">
                        {facet.functionCount ?? 0} funcs
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {/* Function Interface */}
          {contractInfo.verified &&
            (!contractInfo.isDiamond || selectedFacet) && (
              <div className="bg-gray-800 rounded-lg">
                ...
                {/* Facet selector moved here, below tabs */}
                {contractInfo.isDiamond && facets.length > 0 && (
                  <div className="px-6 pt-4">
                    ...
                    <div className="text-xs text-gray-400">
                      {facets.filter((f) => f.verified).length} verified facets
                      • {facets.length} total facets
                    </div>
                  </div>
                )}
      ...
      {!contractInfo && !isLoading && (
        <div className="max-w-6xl mx-auto px-6 py-20 text-center">
          ...
          <p className="text-gray-500">
            Search for any smart contract on supported networks to interact with
            it
          </p>
        </div>
      )}
