...
import { analyzeContract, getContractTypeIcon, getContractTypeDescription } from '../utils/contractAnalyzer';
...
const ContractInfoDisplay: React.FC<ContractInfoDisplayProps> = ({
  ...
  const analysisKey = useMemo(() => {
    ...
    const abiHash = abi.length + (abi.includes('function') ? 'f' : '') + (abi.includes('event') ? 'e' : '');
  ...
  const analyzeContractInfo = useCallback(async () => {
    ...
    try {
      const info = await analyzeContract(abi, contractAddress, chain, memoizedProvider);
  ...
  const getContractTypeIcon = (type: string) => {
    switch (type) {
      case 'ERC20': return <Coins size={20} />;
      case 'ERC721': return <Image size={20} />;
      case 'ERC1155': return <Hash size={20} />;
      case 'PROXY': return <Shield size={20} />;
      case 'MULTISIG': return <Users size={20} />;
  ...
  const getContractTypeColor = (type: string) => {
    switch (type) {
      case 'ERC20': return '#f59e0b'; // Amber
      case 'ERC721': return '#8b5cf6'; // Purple  
      case 'ERC1155': return '#10b981'; // Emerald
      case 'PROXY': return '#3b82f6'; // Blue
      case 'MULTISIG': return '#ef4444'; // Red
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            padding: '4px 12px',
            borderRadius: '20px',
            background: contractInfo.verified ? 'rgba(34, 197, 94, 0.15)' : 'rgba(239, 68, 68, 0.15)',
      ...
      {((contractInfo.contractType === 'ERC20' || contractInfo.contractType === 'ERC721') || contractMetadata?.tokenInfo) && (
        <div style={{ padding: '16px' }}>
          <h5 style={{
            ...
            <Coins size={14} style={{ color: getContractTypeColor(contractInfo.contractType) }} />
            Token Details
          </h5>
          ...
          <div style={{
            ...
            {(contractMetadata?.tokenInfo?.decimals || contractInfo.decimals !== undefined) && (
              <div style={{
                background: 'rgba(255, 255, 255, 0.03)',
                border: '1px solid rgba(255, 255, 255, 0.1)',
                borderRadius: '6px',
                ...
                <div style={{ 
                  ...
                  {contractMetadata?.tokenInfo?.decimals || contractInfo.decimals}
                </div>
              </div>
            )}
            
            {(contractMetadata?.tokenInfo?.totalSupply || contractInfo.totalSupply) && (
              <div style={{
                background: 'rgba(255, 255, 255, 0.03)',
                border: '1px solid rgba(255, 255, 255, 0.1)',
                borderRadius: '6px',
                ...
                <div style={{ 
                  ...
                  <TrendingUp size={12} style={{ color: getContractTypeColor(contractInfo.contractType) }} />
                  {(() => {
                    const totalSupply = contractMetadata?.tokenInfo?.totalSupply || contractInfo.totalSupply;
                    const symbol = contractMetadata?.tokenInfo?.symbol || contractInfo.symbol;
                    const decimals = contractMetadata?.tokenInfo?.decimals || contractInfo.decimals;
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              <Eye size={16} />
              {contractInfo.functions.filter(f => f.type === 'function' && 
                (f.stateMutability === 'view' || f.stateMutability === 'pure')).length}
            </div>
            <div style={{
              fontSize: '11px',
              color: '#9ca3af',
          ...
          <div style={{
            ...
            <div style={{
              ...
              <Edit size={16} />
              {contractInfo.functions.filter(f => f.type === 'function' && 
                (f.stateMutability === 'nonpayable' || f.stateMutability === 'payable')).length}
            </div>
            <div style={{
              fontSize: '11px',
              color: '#9ca3af',
      ...
      {contractInfo.contractType === 'ERC20' && (
        <div style={{
          ...
          <div style={{
            ...
            {['transfer', 'approve', 'balanceOf', 'allowance'].map(funcName => {
              const hasFunction = contractInfo.functions.some(f => f.name === funcName);
              return (
                <span 
                  key={funcName}
                  style={{
                    ...
                    fontWeight: '500',
                    fontFamily: 'Monaco, Menlo, monospace',
                    background: hasFunction ? 'rgba(34, 197, 94, 0.15)' : 'rgba(239, 68, 68, 0.15)',
      ...
      {contractInfo.contractType === 'ERC721' && (
        <div style={{
          ...
          <div style={{
            ...
            {['transferFrom', 'approve', 'ownerOf', 'balanceOf'].map(funcName => {
              const hasFunction = contractInfo.functions.some(f => f.name === funcName);
              return (
                <span 
                  key={funcName}
